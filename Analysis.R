...
##
sdg72019 <- read_excel("Tracking SDG7  - Access to Electricity.xlsx")
sdg72019$iso3 = countrycode(sdg72019$Country, "country.name", "iso3c")
pr = merge(merged_diff, sdg72019, by.x="GID_0", by.y="iso3")

correlation <- (cor(pr$elrate18, pr$`2017`))^2

comparison_progress <- View(data.frame(pr$elrate_diff, pr$CH, pr$Country))

#3.1) Plot change in national electricity access rates between the two years considered

barplot_elrate = ggplot(merged_diff, aes(x=reorder(GID_0, -elrate_diff), y=elrate_diff))+
      theme_classic()+
...
#3.1) Plot change in national electricity access rates between the two years considered
barplot_elrate_rur = ggplot(merged_diff_rur, aes(x=reorder(GID_0, -elrate_diff), y=elrate_diff))+
  theme_classic()+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  geom_bar(stat="identity", aes(fill=elrate18))+
...
#3.1) Plot change in national electricity access rates between the two years considered
barplot_elrate_urb = ggplot(merged_diff_urb, aes(x=reorder(GID_0, -elrate_diff), y=elrate_diff))+
  theme_classic()+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  geom_bar(stat="identity", aes(fill=elrate18))+
  scale_fill_gradient2(low="firebrick2", mid="gold", midpoint = 0.5, high="forestgreen", name="2018",  labels = scales::percent_format(accuracy = 1))+
...
pgrid = plot_grid(lorenz[['RWA']] + theme(legend.position="none"), lorenz[['KEN']] + theme(legend.position="none"), lorenz[['ETH']] + theme(legend.position="none"), lorenz[['NGA']]  + theme(legend.position="none"), lorenz[['COD']] + theme(legend.position="none"), lorenz[['MOZ']] + theme(legend.position="none"), labels=c("RWA", "KEN", "ETH", "NGA", "COD", "MOZ"), label_size = 8, hjust= 0, vjust = 1, ncol=2)

ylab <- ggdraw() + draw_label("Cum. percentage of national electr. access", size = 10, angle = 90)
...
xlab <- ggdraw() + draw_label("Cum. percentage of popul. from lowest to highest electrified", size = 10)
...
ggsave("Pie_consumption_rural.pdf", plot = Pie_consumption_rural, device = "pdf", width = 28, height = 15, units = "cm", scale=0.7)
...
tit_a <- ggdraw() + draw_label("Split of \n consumption tiers", size = 13, angle = 90)
...
ggsave("Pie_consumption_urban.pdf", plot = Pie_consumption_urban, device = "pdf", width = 24, height = 15, units = "cm", scale=0.7)
